...
# Class order must match the YAML used during training:
# YAML:
# 0: barcode
# 1: brand
# 2: price
...
def predict_on_image(model, image_path, conf_threshold=0.5):
    ...
    if len(detections) > 0:
        for det in detections:
            # Get box coords as a flat array [x1, y1, x2, y2]
            coords = det.xyxy.cpu().numpy().flatten()
            x1, y1, x2, y2 = map(int, coords)

            # Get class name
            cls_id = int(det.cls.cpu().numpy().item())
            cls_name = CLASS_NAMES[cls_id]

            # Crop the region
            cropped = crop_region(original_img, (x1, y1, x2, y2))
...
if __name__ == "__main__":
    ...
    model_path = "/Users/allenpereira/Desktop/d33ewd 2/runs/detect/new_run19/weights/best.pt"
    test_image_path = "/Users/allenpereira/Desktop/d33ewd 2/WhatsApp Image 2025-03-16 at 7.17.52 PM.jpeg"
